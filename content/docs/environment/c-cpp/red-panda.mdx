---
title: 小熊猫 Dev-C++
---

<Callout type="warning">
本条目仍在积极施工中，可能存在遗漏或不完善的地方
</Callout>

![小熊猫C++](./images/red-panda/rc-1.png)

## 简介

小熊猫编译器（RedPanda-CPP）是轻量级的 C/C++ IDE 。它集成了GNU的GCC、G++编译器以及GDB调试器。它以简洁的用户界面、多平台支持（Windows、Mac OS、Linux）和高效的代码编辑功能为特点，适合编程初学者和教育用途。

小熊猫编译器提供了代码自动补全、语法高亮、代码导航等辅助工具，允许用户根据项目需求配置编译选项。

## 下载

### 官网下载

[小熊猫官网](http://royqh.net/redpandacpp/download/)一共提供了三种下载渠道，这里只提供两种不限速的渠道：


- [官方SourceForge仓库](https://sourceforge.net/projects/redpanda-cpp/files/v3.1/RedPanda.C%2B%2B.3.1.win64.MinGW64_11.4.Setup.exe/download)

:::        

 点开直接下载即可
 ![小熊猫C++-2](./images/red-panda/newP8.png)
- [官方蓝奏云 - (密码：f0tp)](https://wwe.lanzoui.com/b01os0mwd)

::: 

**打开官网**，点击下载页面
![小熊猫C++-2](./images/red-panda/newP0.png)

点击第一个蓝奏云网盘下载
![小熊猫C++-2](./images/red-panda/newP1.png)

info :clipboard:Tips
蓝奏云下载方式请注意下载下图中所指示的版本

![小熊猫C++-2](./images/red-panda/newP2.png)

输入提取码-f0tp
![小熊猫C++-2](./images/red-panda/newP3.png)

任点一个进行下载
![小熊猫C++-2](./images/red-panda/newP4.png)

*下载完成后有时会出现风险问题（点击该文件右上角三个点）
![小熊猫C++-2](./images/red-panda/newP5.png)

接着点击保留
![小熊猫C++-2](./images/red-panda/newP6.png)

然后会出现弹窗，找到仍然保留即可正常打开文件
![小熊猫C++-2](./images/red-panda/newP7.png)


### 本站下载

[点击此处下载](/download)

## 安装

打开下载好的安装程序，等待加载完成，之后选择`简体中文`，并点击`OK`
![install1.jpg](./images/red-panda/install1.png)


用户协议选择 `我接受`
![install2.jpg](./images/red-panda/install2.jpg)

安装的用户选择 `只为我自己安装`

![install3.png](./images/red-panda/install3.png)

`选择组件`一般默认就行
![install4.jpg](./images/red-panda/install4.jpg)

接下来的`安装路径`自己选择即可，没有的话默认也可以
![install5.png](./images/red-panda/install5.png)

等待`正在安装`中
![install6.jpg](./images/red-panda/install6.jpg)

点击`完成`即安装结束

![install7.jpg](./images/red-panda/install7.jpg)

点击 `yes` 后开始设置`主题`，`语言`（按照需求安装）

![使用1.png](./images/red-panda/使用1.png)

至此，我们的安装步骤完成

![使用2.png](./images/red-panda/使用2.png)

## 使用

### 创建项目

首先点击 `文件`

![使用3.png](./images/red-panda/使用3.png)

选择 `新建` 然后点击 `创建新的C/C++文件` (也可以先创建一个项目然后在项目里创建C/C++文件)。

项目相当于一个文件夹，如果有很多相同性质的C/C++文件可以放在一个项目里。

::: info :clipboard:Tips
但多说一句，项目的实际作用不止于此：它是一个完整的工程，里面会有多个完成特定功能的模块。感兴趣的同学可以在空余时间了解一下：在编程领域中，什么是项目。
:::

这里用创建项目做演示：点击OK后会提示不存在该项目（如果存在说明重名了）

![使用4.png](./images/red-panda/使用4.png)

随后创建项目文件

![使用5.png](./images/red-panda/使用5.png)

![使用6.png](./images/red-panda/使用6.png)

### 运行代码

现在就可以编写第一个程序了

![使用7.png](./images/red-panda/使用7.png)

这是源代码

```c lineNumbers title="example.c" title="example.c"
#include<stdio.h>
int main(){
	int a,b;
	scanf("%d,%d",&a,&b);
	printf("两数之和为%d",a+b);
	return 0;
}
```
运行结果

![使用8.png](./images/red-panda/使用8.png)

## 配置

在顶部菜单栏中，选择 `工具` - `选项`，找到窗口左侧列表中的外观

![使用9.png](./images/red-panda/使用9.png)

字体、背景、语法高亮等常见外观都可以在这里调整。

<Callout title="Tips">
用到这里是不是感觉 小熊猫C++ 和 Dev-C++ 很相似呢？其实这两者之间有很大的联系。你可以简单地理解为：小熊猫C++ 是在 Dev-C++ 停止更新后的接替品。
</Callout>
