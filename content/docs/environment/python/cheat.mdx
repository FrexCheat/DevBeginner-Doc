...
第一行 `size` （表示链表长度，0 < `size` {"<="} 1000 ）；

第二行输入 `size` 个数，为每个节点对应值 `val` （ |`val`| {"<="} 1000）；

第三行为 `m`，`n` （为反转区间的起止位置，0 < `m` {"<="} `n` {"<="} `size`）。
...
显然考官希望我们用 C 语言结构体指针或者 C++ 的 STL 标准库来解决问题。这道题 C 的标准答案如下：

```c lineNumbers title="example.c"
#include<stdio.h>
#include<assert.h>
#include<stdlib.h>
